script_author("Shincan x 4trays SecureCode")
script_version("2.0 test")
...
local function drawKasirTab()
    ...
    if imgui.BeginChild("##serviceList", imgui.ImVec2(0, listH), true) then
        local colW = (ww - 40) / 2 - 4
        for i, svc in ipairs(serviceList) do
            if (i % 2) ~= 1 then imgui.SameLine(colW + 20) end
            ...
            if imgui.BeginChild("##svc"..i, imgui.ImVec2(colW, 36), false) then
    ...
    if imgui.BeginChild("##totalArea", imgui.ImVec2(0, 68), true) then
...
local function drawSettingsTab()
    ...
    imgui.Spacing()
    imgui.TextWrapped(fa.INFO .. " /mekanik or /mh = buka/tutup   |   /mekareload = reload JSON")
    imgui.PopStyleColor()
end
...
function main()
    ...
    sampRegisterChatCommand("mekanik", function()
    ...
    sampAddChatMessage("[Mechanic Helper] {FFFFFF}Loaded! Ketik {4D94FF}/mekanik {FFFFFF}atau {4D94FF}/mh {FFFFFF}untuk membuka.", 0x4D94FF)
